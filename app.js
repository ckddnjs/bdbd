...
/* ── 연장 신청: 대기 중인 예약 없으면 +3일 가능 ── */
async function requestExtend(rentalId, gameId, currentDueDate) {
  // 해당 게임에 pending 대기자가 있는지 확인
  const {data:waiting} = await sb.from('rentals')
    .select('id').eq('game_id', gameId).eq('status','pending');
  if (waiting && waiting.length > 0) {
    showToast('⚠️ 대기 중인 분이 있어 연장이 불가해요');
    return;
  }
  const newDue = new Date(currentDueDate);
  newDue.setDate(newDue.getDate()+3);
  await sb.from('rentals').update({due_date: newDue.toISOString()}).eq('id', rentalId);
  loadRentalStatus();
  showToast('📅 반납일이 3일 연장됐어요!');
}
...
async function loadRentalStatus() {
  ...
  // 각 게임별 pending 대기자 수 가져오기
  const {data:pendingAll} = await sb.from('rentals').select('game_id').eq('status','pending');
  const pendingMap = {};
  (pendingAll||[]).forEach(r => { pendingMap[r.game_id] = (pendingMap[r.game_id]||0)+1; });

  const statusMap = {pending:'⏳ 대기중', approved:'📦 대여중', returned:'✅ 반납'};
  const pillMap   = {pending:'p-yellow',  approved:'p-red',     returned:'p-green'};

  el.innerHTML = all.map(r => {
    const hasWaiting = (pendingMap[r.game_id]||0) > 0 && r.status==='approved';
    const canExtend  = r.status==='approved' && !hasWaiting;
    return `
    <div class="ritem">
      ...
      ${r.due_date ? `
        <div style="font-size:.75rem;color:var(--text2);display:flex;align-items:center;gap:.5rem;flex-wrap:wrap">
          <span>반납예정 ${fmt(r.due_date)}</span>
          ${canExtend ? `<button class="btn btn-sm" style="padding:.25rem .65rem;font-size:.68rem;background:var(--blue);color:white;border-radius:var(--rfull)" onclick="requestExtend('${r.id}','${r.game_id}','${r.due_date}')">+3일 연장</button>` : ''}
          ${hasWaiting ? `<span style="color:var(--orange);font-size:.68rem">⚠️ 대기자 있음 (연장 불가)</span>` : ''}
        </div>` : ''}
...
async function loadApproveList() {
  ...
  if (!pending||!pending.length) { el.innerHTML='<div class="empty"><div class="ei">🎉</div><p>대기 중인 신청이 없어요</p></div>'; return; }
...
async function approveRental(rentalId, gameId, days) {
  ...
  await sb.from('rentals').update({status:'approved', approved_at:new Date().toISOString(), due_date:due.toISOString()}).eq('id',rentalId);
...
async function addGame() {
  ...
  if (error) { showToast('오류: '+error.message); return; }
  ['ag-title','ag-desc','ag-yt','ag-img-url','ag-minp','ag-maxp','ag-mint','ag-maxt']
    ...
    <div class="ag-row" id="agr-${g.id}">
      ...
      <div style="flex:1;min-width:0">
        ...
        <div style="font-size:.65rem;color:var(--text3);margin-top:.1rem">
          ${g.genre} · 👥${g.min_players}~${g.max_players} · ⏱${g.min_time}~${g.max_time}분 · ${'★'.repeat(g.difficulty)}
        </div>
      </div>
      <span class="pill ${g.is_available?'p-green':'p-red'}" style="flex-shrink:0">${g.is_available?'가능':'대여중'}</span>
      <div class="ag-acts">
        <button class="btn btn-yellow btn-sm" onclick="openEditGame('${g.id}')">✏️</button>
...
async function openEditGame(id) {
  const {data:g} = await sb.from('games').select('*').eq('id',id).single();
  if (!g) return;

  editImgUrl  = g.image_url||'';
  editDiffVal = g.difficulty||1;

  // 폼 채우기
  document.getElementById('eg-id').value    = g.id;
  document.getElementById('eg-title').value = g.title;
  document.getElementById('eg-minp').value  = g.min_players;
  document.getElementById('eg-maxp').value  = g.max_players;
  document.getElementById('eg-mint').value  = g.min_time;
  document.getElementById('eg-maxt').value  = g.max_time;
  document.getElementById('eg-genre').value = g.genre;
  document.getElementById('eg-desc').value  = g.description||'';
  document.getElementById('eg-yt').value    = g.youtube_url||'';
  document.getElementById('eg-avail').value = g.is_available?'true':'false';

  // 이미지 미리보기
  const dropEl = document.getElementById('eg-img-drop');
  if (g.image_url) {
    dropEl.innerHTML=`<img src="${esc(g.image_url)}"><div class="img-drop-txt" style="background:rgba(0,0,0,.4);color:white;width:100%;text-align:center;padding:.3rem;position:relative;z-index:1">변경하려면 탭</div>`;
  } else {
    dropEl.innerHTML=`<span style="font-size:1.8rem">📸</span><div class="img-drop-txt">탭하여 이미지 업로드</div>`;
  }

  // 난이도 별점
  setEditDiff(g.difficulty||1);

  document.getElementById('edit-game-modal').classList.add('open');
  document.body.style.overflow = 'hidden';
}

function setEditDiff(v) {
  editDiffVal = v;
  document.getElementById('eg-diff').value = v;
  document.querySelectorAll('#eg-star-row .star-btn').forEach(b=>b.classList.toggle('on',+b.dataset.v<=v));
}

async function handleEditImgUpload(input) {
  const file = input.files[0];
  if (!file) return;
  showToast('📸 이미지 업로드 중...');
  const ext  = file.name.split('.').pop();
  const path = `edit_${Date.now()}.${ext}`;
  const {error} = await sb.storage.from('game-images').upload(path, file, {upsert:true});
  if (error) { showToast('업로드 실패: '+error.message); return; }
  const {data:{publicUrl}} = sb.storage.from('game-images').getPublicUrl(path);
  editImgUrl = publicUrl;
  const drop = document.getElementById('eg-img-drop');
  drop.innerHTML=`<img src="${publicUrl}"><div class="img-drop-txt" style="background:rgba(0,0,0,.4);color:white;width:100%;text-align:center;padding:.3rem;position:relative;z-index:1">변경하려면 탭</div>`;
  showToast('✅ 이미지 업로드 완료!');
}
...
async function saveEditGame() {
  ...
  const minP  = +document.getElementById('eg-minp').value;
  const maxP  = +document.getElementById('eg-maxp').value;
  const minT  = +document.getElementById('eg-mint').value;
  const maxT  = +document.getElementById('eg-maxt').value;
  if (!title||!minP||!maxP||!minT||!maxT) { showToast('필수 항목을 모두 입력해주세요'); return; }
  ...
  const {error} = await sb.from('games').update({
    ...
    min_players:  minP, max_players: maxP,
    min_time:     minT, max_time:    maxT,
    genre:        document.getElementById('eg-genre').value,
    difficulty:   +document.getElementById('eg-diff').value||1,
    description:  document.getElementById('eg-desc').value.trim()||null,
    youtube_url:  document.getElementById('eg-yt').value.trim()||null,
    is_available: document.getElementById('eg-avail').value==='true',
  }).eq('id', id);

  if (error) { showToast('오류: '+error.message); return; }
  closeEditGameModal();
  loadAdminGameList();
  showToast('✅ 게임 정보가 수정됐어요!');
}

function closeEditGameModal(e) {
  if (!e||e.target===document.getElementById('edit-game-modal')) {
...
function initCsvDrop() {
  ...
  fileInput.addEventListener('change', e=>{ if(e.target.files[0]) handleCsvFile(e.target.files[0]); });
}
...
function parseGenre(s) {
  for (const [k,v] of Object.entries(GENRE_MAP)) { if(s.includes(k)) return v; }
  return '파티';
}
